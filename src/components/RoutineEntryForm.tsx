...
import { detectConflicts, detectExpiryWarnings, type ConflictMatch } from '../data/conflicts';
...
export default function RoutineEntryForm({ products, session, onSubmit, onCancel, initial }: Props) {
  const [productIds, setProductIds] = useState<string[]>(initial?.productIds ?? []);
  const [skinRating, setSkinRating] = useState<SkinRating | null>(initial?.skinRating ?? null);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="flex items-center gap-2">
        <span className={`rounded-full px-3.5 py-1 text-xs font-bold backdrop-blur
          ${session === 'AM'
      ...
      {expiredProducts.length > 0 && (
        <div className="rounded-xl bg-red-500/10 backdrop-blur border border-red-400/20 dark:border-red-500/15 px-3.5 py-2.5 text-xs">
          <span className="font-bold text-red-600 dark:text-red-400">🚫 Expired products</span>
          <p className="mt-0.5 text-red-700/80 dark:text-red-300/80 leading-relaxed">
            {expiredProducts.map((p) => p.name).join(', ')} {expiredProducts.length === 1 ? 'has' : 'have'} passed
            {expiredProducts.length === 1 ? ' its' : ' their'} PAO expiry date. Consider replacing before use.
          </p>
        </div>
      )}
