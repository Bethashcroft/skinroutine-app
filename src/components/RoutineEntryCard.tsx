...
export default function RoutineEntryCard({ entry, products, onEdit, onDelete }: Props) {
  ...
  return (
    <div className="card overflow-hidden transition-all duration-300 hover:-translate-y-0.5">
      <div className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-3.5 border-b border-white/15 dark:border-white/8">
        <span className={`rounded-full px-3 py-1 text-xs font-bold backdrop-blur
          ${entry.session === 'AM'
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-2xl" title={`Skin rating: ${entry.skinRating}/5`}>
            {RATING_EMOJI[entry.skinRating]}
          </span>
          <span className="text-xs font-bold text-gray-400 dark:text-gray-500">{entry.skinRating}/5</span>
        </div>
      </div>
      ...
      <div className="px-4 sm:px-5 py-3.5 sm:py-4">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {usedProducts.length === 0 && (
            <span className="text-sm italic text-gray-300 dark:text-gray-600">Products removed from library</span>
          )}
        </div>

        {entry.notes && (
          <p className="mt-3 rounded-xl bg-white/25 dark:bg-white/5 backdrop-blur
      ...
      <div className="flex justify-end gap-2 border-t border-white/15 dark:border-white/8 px-4 sm:px-5 py-2.5 sm:py-3">
        <button type="button" onClick={onEdit} className="btn-ghost px-4! py-1.5! text-xs!">
          Edit
        </button>
        {confirmDelete ? (
