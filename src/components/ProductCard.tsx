...
function getExpiryInfo(product: Product): { label: string; status: 'ok' | 'warning' | 'expired' } | null {
...
export default function ProductCard({ product, isExpanded, onToggle, onEdit, onDelete, stats, children }: Props) {
  ...
  return (
    <div className={`card overflow-hidden transition-all duration-300
      ...
      <div className="flex">
        ...
        <div className={`w-1 shrink-0 ${ACCENT_COLORS[product.category] ?? 'bg-sand-400'}`} />
            ...
            <div className="mt-2 flex items-center gap-2 flex-wrap">
              {CATEGORY_ORDER.filter((c) => flagCounts.has(c)).map((cat) => (
                <span key={cat} className="flex items-center gap-1"
                  ...
                  <span className="text-[11px] font-bold text-gray-400 dark:text-gray-500">{flagCounts.get(cat)}</span>
                </span>
              ))}
              {expiry && (
                ...
                >
                  {expiry.status === 'expired' ? '⚠️ ' : ''}{expiry.label}
                </span>
              )}
            </div>
          </div>
      ...
      {isExpanded && (
        <div className="border-t border-white/15 dark:border-white/8 px-5 py-4 sm:py-5">
            ...
            <div className="mb-4 space-y-2.5">
              {stats && stats.timesUsed > 0 && (
                <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                  <span className="flex items-center gap-1">
                    ...
                    Used <strong className="text-gray-700 dark:text-gray-200">{stats.timesUsed}</strong> time{stats.timesUsed !== 1 ? 's' : ''}
                  </span>
                  {stats.lastUsed && (
                    <span className="flex items-center gap-1">
                      · Last used <strong className="text-gray-700 dark:text-gray-200">{formatLastUsed(stats.lastUsed)}</strong>
                    </span>
                  )}
                </div>
              )}
              {expiry && (
                <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                  <span className="text-sm">🧴</span>
                  Opened {new Date(product.openedAt! + 'T00:00:00').toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                  <span className="mx-1">·</span>
                  {product.paoMonths}M PAO
                  <span className="mx-1">·</span>
                  <span className={expiry.status === 'expired' ? 'font-bold text-red-500 dark:text-red-400' : expiry.status === 'warning' ? 'font-bold text-amber-600 dark:text-amber-400' : ''}>
                    {expiry.label}
                  </span>
                </div>
              )}
              {product.notes && (
                <p className="inline-block rounded-lg bg-white/25 dark:bg-white/5 backdrop-blur
          ...
          <div className="mt-4 sm:mt-5 flex justify-end gap-2 border-t border-white/15 dark:border-white/8 pt-3 sm:pt-4">
            <button type="button" onClick={handleShare} className="btn-ghost px-4! py-2! text-xs! mr-auto">
              {copied ? '✓ Link copied' : 'Share'}
            </button>
            <button type="button" onClick={onEdit} className="btn-ghost px-4! py-2! text-xs!">
              Edit
            </button>
            {confirmDelete ? (
