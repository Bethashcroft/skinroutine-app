...
> = {
  fragrance: {
    ...
    description: 'EU-regulated scent compounds that can cause sensitivity',
  },
  irritant: {
    emoji: '🟠',
    label: 'Potential Irritants',
    description: 'Ingredients that may irritate sensitive skin',
  },
  comedogenic: {
    emoji: '🟡',
    label: 'Comedogenic',
    description: 'Ingredients known to clog pores',
  },
  active: {
    emoji: '🔵',
    label: 'Active Ingredients',
    description: 'Beneficial actives worth tracking',
  },
};
...
const PICKER_OPTIONS: FlagOption[] = [
  ...
  {
    key: 'oakmoss',
    label: 'Evernia Prunastri (Oakmoss)',
    category: 'fragrance',
  },
  {
    key: 'treemoss',
    label: 'Evernia Furfuracea (Treemoss)',
    category: 'fragrance',
  },
  {
    key: 'methyl-2-octynoate',
  ...
  {
    key: 'isopropyl-myristate',
    label: 'Isopropyl Myristate',
    category: 'comedogenic',
  },
  {
    key: 'isopropyl-palmitate',
    label: 'Isopropyl Palmitate',
    category: 'comedogenic',
  },
...
export function getPickerOptionsByCategory(): {
  category: FlagCategory;
  options: FlagOption[];
}[] {
  const order: FlagCategory[] = [
    'active',
    'fragrance',
    'irritant',
    'comedogenic',
...
export function flagsToOptionKeys(flags: IngredientFlag[]): string[] {
  return flags
    .map((f) => {
      const opt = PICKER_OPTIONS.find(
        (o) => o.label === f.ingredient && o.category === f.category,
      );
      return opt?.key;
    })
