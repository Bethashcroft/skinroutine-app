...
function getGreeting() {
  ...
  if (h < 12)
    return {
      text: 'Good morning',
      emoji: '☀️',
      sub: 'Time for your morning routine',
    };
  if (h < 17)
    return {
      text: 'Good afternoon',
      emoji: '🌤️',
      sub: 'How is your skin feeling today?',
    };
  if (h < 21)
    return {
      text: 'Good evening',
      emoji: '🌙',
      sub: 'Wind down with your evening routine',
    };
  return {
    text: 'Good night',
    emoji: '🌙',
    sub: 'Remember your evening routine',
  };
}
...
export default function Dashboard() {
  ...
  const { entries, getEntriesForDate, getStreak, getLast7DaysRatings } =
    useRoutineLog();
  ...
  const handleAddRecommendation = useCallback(
    (rec: (typeof recommendations)[number]['product']) => {
  ...
  const flagWarnings = useMemo(() => {
    ...
    const warnings: {
      product: Product;
      flags: { ingredient: string; category: FlagCategory }[];
    }[] = [];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl">
        <div
          className="absolute inset-0 bg-linear-to-br from-sand-500 via-sand-600 to-sand-700
        ...
        <div className="relative px-6 sm:px-8 py-7 sm:py-9 text-white">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              <p className="text-[13px] font-medium text-white/60 tracking-wide uppercase">
                {dateLabel}
              </p>
            ...
            {streak > 0 && (
              <div
                className="flex items-center sm:flex-col gap-2 sm:gap-0.5 rounded-2xl bg-white/15 backdrop-blur-md
        ...
        {!profile && !editingProfile ? (
          <div className="px-5 sm:px-6 py-5 text-center sm:text-left">
            <p className="text-sm text-gray-400 dark:text-gray-500 leading-relaxed">
              Tell us your skin type and concerns to get personalised product
              recommendations.
            </p>
            <button
              type="button"
              ...
              >
                ...
                <button
                  ...
                  className="shrink-0 rounded-lg border px-3 py-1.5 text-xs font-semibold transition-colors
                             bg-sand-500/15 dark:bg-sand-400/10 border-sand-400/25 dark:border-sand-500/15
                             text-sand-700 dark:text-sand-300 hover:bg-sand-500/25 dark:hover:bg-sand-400/20"
                >
                  + Add
                </button>
              </div>
        ...
        <section className="card-solid noise overflow-hidden">
          <div className="px-5 sm:px-6 py-6 sm:py-8 text-center">
            ...
            <p className="mt-2 text-sm text-gray-400 dark:text-gray-500 max-w-xs mx-auto leading-relaxed">
              Get started by adding the products you use, then log your first
              routine.
            </p>
      ...
      <section className="card-solid noise overflow-hidden">
        ...
        {!amEntry && !pmEntry ? (
          <div className="flex flex-col items-center px-5 py-14">
            <div
              className="mb-4 h-16 w-16 rounded-2xl bg-white/30 dark:bg-white/8 backdrop-blur border border-white/20
            ...
            <p className="mt-1 text-sm text-gray-400 dark:text-gray-500">
              Track your skincare to build your streak
            </p>
      ...
      <section className="card-solid noise overflow-hidden">
        ...
        {!hasData ? (
          <div className="flex flex-col items-center px-5 py-10">
            <div
              className="mb-3 h-12 w-12 rounded-xl bg-white/25 dark:bg-white/8 backdrop-blur border border-white/15
                            flex items-center justify-center"
            >
              <span className="text-xl">📈</span>
            </div>
            <p className="text-sm text-gray-400 dark:text-gray-500">
              Start logging to see your skin trend
            </p>
          </div>
        ) : (
                ...
                >
                  ...
                  <Tooltip
                    ...
                    content={({ active, payload }) => {
                      ...
                      return (
                        <div
                          className="rounded-xl px-4 py-2.5 text-sm shadow-xl backdrop-blur-xl
        ...
        <Toast
          ...
          message={
            <>
              ...
              has already been added. Try entering another or edit it in your
              library.
            </>
          }
                ...
                <div>
                  ...
                  <div className="mt-1.5 flex flex-wrap gap-1">
                    {flags.map((f, i) => (
                      <span
                        key={i}
                        className="rounded-full bg-red-100/50 dark:bg-red-900/30 backdrop-blur
  ...
  return (
    <div className="flex items-center gap-3 py-3.5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <span className="block text-xs text-gray-400 dark:text-gray-500">
          {status === 'expired' ? 'Past PAO — replace' : 'Expires soon'} ·{' '}
          {label}
        </span>
      </div>
      <Link
        to="/products"
        className="text-xs font-semibold text-sand-600 dark:text-sand-400 shrink-0"
      >
        View
      </Link>
    </div>
...
function StatPill({
  ...
  return (
    <div
      className="shrink-0 flex items-center gap-3 rounded-2xl px-5 py-3.5 backdrop-blur-xl
    ...
    >
      ...
      <div>
        <span className="block text-2xl font-black text-gray-800 dark:text-gray-100 tracking-tight">
          {value}
        </span>
        <span className="block text-[11px] font-medium text-gray-400 dark:text-gray-500 -mt-0.5">
          {label}
        </span>
      </div>
    </div>
...
function SessionSummary({
  ...
  if (!entry) {
    return (
      <div
        className={`flex flex-col items-center px-4 py-6 ${isSecond ? 'border-t sm:border-t-0 sm:border-l border-white/10 dark:border-white/5' : ''}`}
      >
  ...
  return (
    <div
      className={`px-5 py-4 ${isSecond ? 'border-t sm:border-t-0 sm:border-l border-white/10 dark:border-white/5' : ''}`}
    >
      ...
      {entry.notes && (
        <p
          className="mt-2.5 inline-block rounded-lg bg-white/25 dark:bg-white/5 backdrop-blur
